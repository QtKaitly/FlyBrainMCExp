...
public final class SensoryEncoders {
    public static final class Params {
        ...
        public double joAuditoryRMax = 150, joWindRMax = 180, joWindSpont = 0, joGroomRMax = 150;
        ...
        public boolean vision = true, olfaction = true, gustation = true, mechanosensation = true, thermoHygro = true;
        /**
    ...
    private void applyMechanosensation(SensoryFrame f, LifNetwork net) {
        ...
        setRates(net, joAuditoryL, p.joAuditoryRMax * hill(sound));
        setRates(net, joAuditoryR, p.joAuditoryRMax * hill(sound));
        // JO-B (<100 Hz) shares the auditory pool in this dataset's annotation; low band adds to both
        if (f.soundLow > 0) {
            setRates(net, joAuditoryL, p.joAuditoryRMax * hill(Math.max(sound, f.soundLow * 0.7)));
            setRates(net, joAuditoryR, p.joAuditoryRMax * hill(Math.max(sound, f.soundLow * 0.7)));
        }

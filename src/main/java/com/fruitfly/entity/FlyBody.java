...
public final class FlyBody {
    ...
    public static void travel(FlyEntity fly, MotorDecoder.MotorCommand cmd, SensoryFrame frame, FruitFlyConfig cfg, State st) {
        ...
        boolean locomotorSilence = fwd < 0.05 && Math.abs(yaw) < 0.05 && bwd < 0.05;
        ...
        if (cfg.reflexLayer && locomotorSilence && !stationaryMode && !st.flying && frame != null) {
            ...
            } else if (!Float.isNaN(frame.odorBearingDeg) && good > 0.08 && good > bad) {
                // chemotaxis: turn toward the odor bearing, walk when roughly aligned
                double b = frame.odorBearingDeg;
                yaw = Mth.clamp(b / 45.0, -1, 1) * 0.7;
                fwd = Math.abs(b) < 60 ? 0.35 * Math.min(1, good * 2) : 0.05;
                st.reflexDriving = true;
            } else if (bad > 0.3 && !Float.isNaN(frame.odorBearingDeg)) {
                // aversion: turn away and walk
                double b = frame.odorBearingDeg;
                yaw = -Math.signum(b == 0 ? 1 : b) * 0.7;
                fwd = 0.3;
                st.reflexDriving = true;
            } else if (!brainPresent || mode == MotorDecoder.Mode.IDLE) {
                // exploration bouts: real flies alternate walking bouts and pauses
                st.exploreTimer -= dt;
                if (st.exploreTimer <= 0) {
                    boolean walk = st.random.nextDouble() < 0.45;
                    st.boutForward = walk ? 0.2 + 0.3 * st.random.nextDouble() : 0;
                    st.boutYaw = walk ? (st.random.nextDouble() - 0.5) * 0.9 : (st.random.nextDouble() < 0.3 ? (st.random.nextDouble() - 0.5) : 0);
                    st.exploreTimer = 0.8 + 2.5 * st.random.nextDouble();
                }
                fwd = st.boutForward;
                yaw = st.boutYaw;
                st.reflexDriving = fwd > 0 || yaw != 0;
            }
        }
        if (fly.horizontalCollision && st.collisionTurnTicks <= 0 && !st.flying) {
            st.collisionTurnTicks = 8;
        ...
        if (jumpNow) {
            ...
            double friction = fly.onGround() ? 0.55 : 0.9;
            v = new Vec3(v.x * friction + target.x * (1 - friction), vy, v.z * friction + target.z * (1 - friction));
            if (fly.onGround() && f > 0.05 && fly.horizontalCollision && fly.tickCount % 4 == 0) v = v.add(0, 0.25 * scale, 0); // step up
        }
